...
def make_rotation_matrix(dim,
    ...
    if convention == 'from_euler':
        if (ori.size == 3):  # degenerate case with a single grain
            # duplicate single crystal orientation in order to get a 2D list as well
            ori = [ori, ori]
    ...
    phi1, Phi, phi2 = Function(V, name='phi1'), Function(
        V, name='Phi'), Function(V, name='phi2')
...
R, phi1, Phi, phi2 = make_rotation_matrix(
    3, 'meshes/n20-id1.ori', Vmf, grains, convention='from_euler')
...
bcs = [
    ...
    DirichletBC(Vu, Constant((0, 0, 0)),
                CompiledSubDomain(
        'near(x[0], 0.)*near(x[1], 0.)*near(x[2], 0.)'),
        method='pointwise'),
    ...
    DirichletBC(Vu.sub(1), Constant(0),
                CompiledSubDomain(
                    'near(x[0], 1.)*near(x[1], 0.)*near(x[2], 1.)'),
                method='pointwise'),
]
...
for (b, behaviour) in enumerate(behaviours):
    ...
    material = mf.MFrontNonlinearMaterial("./src/libBehaviour.so",
                                          ...
                                          rotation_matrix=R.T)
    problem = mf.MFrontNonlinearProblem(
        u, material, quadrature_degree=1, bcs=bcs)
    '''
    ...
    file_results = XDMFFile(
        "results/{}_{}_polycrystal_results.xdmf".format(behaviour, hypothesis))
    ...
    for (i, t) in enumerate(load_steps):
        ...
        if behaviour == "FCCMericCailletaudSingleCrystalViscoPlasticity":
            epsel = problem.get_state_variable(
                "ElasticStrain", project_on=("DG", 0), as_tensor=True)
            epspl = problem.get_state_variable(
                "PlasticStrain", project_on=("DG", 0), as_tensor=True)
            strain = problem.get_gradient(
                "Strain", project_on=("DG", 0), as_tensor=True)
            cauchy = problem.get_flux(
                "Stress", project_on=("DG", 0), as_tensor=True)
            ...
            Fp = problem.get_state_variable(
                "PlasticPartOfTheDeformationGradient", project_on=("DG", 0), as_tensor=True)
            Fe = problem.get_state_variable(
                "ShiftedElasticPartOfTheDeformationGradient", project_on=("DG", 0), as_tensor=True)
            F = problem.get_gradient(
                "DeformationGradient", project_on=("DG", 0), as_tensor=True)
            PK1 = problem.get_flux(
                "FirstPiolaKirchhoffStress", project_on=("DG", 0), as_tensor=True)
        ...
        for s in range(12):
            PlasticSlip = problem.get_state_variable(
                "PlasticSlip[%s]" % s, project_on=("DG", 0))
            EquivalentViscoplasticSlip = problem.get_state_variable(
                "EquivalentViscoplasticSlip[%s]" % s, project_on=("DG", 0))
            BackStrain = problem.get_state_variable(
                "BackStrain[%s]" % s, project_on=("DG", 0))
            ResolvedShearStress = problem.get_state_variable(
                "ResolvedShearStress[%s]" % s, project_on=("DG", 0))
...
plt.plot(results[0, :, 0], results[0, :, 1]/tau0,
         "-o", markevery=3, label="Small strains")
plt.plot(results[1, :, 0], results[1, :, 1]/tau0,
         "-s", markevery=3, label="Finite strains")
plt.xlabel(xlabel)
plt.ylabel(ylabel)
plt.xlim([0., 0.25])
plt.ylim([0., 4.0])
